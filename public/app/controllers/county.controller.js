    ...
    function ($scope, linksService, $location) {
        ...
        function parseDates (array) {
            for(var i = 0; i < array.length; i++) {
                array[i].DTADD = moment(array[i].DTADD ).format('MMM DD, YYYY');
                array[i].openDateIsLaterThanToday = moment(array[i].OPENDATE).isAfter(moment()); // true
            }
        }
        ...
        function loadRentals(town) {
            ...
            linksService.getRentalProperties(vm.searchConfig.orderBy, vm.offset, vm.searchConfig.sortByAsc, vm.county, safeSelectedTown, 20, vm.baths, vm.bathsPart, vm.bedrooms, vm.priceMin, vm.priceMax).then(function (resp) {
                vm.loading = false;
                vm.properties = resp.data;
                countTotal();
                getGeoCodesFromProperties();
        ...
        function countTotal() {
            var safeSelectedTown = (vm.selectedTown === 'All Towns') ? 'A' : vm.selectedTown;
            vm.countLoading = true;
        ...
        function getAvailableTowns() {
            linksService.getAvailableTownsByCounty(vm.county).then(function (resp) {
                ...
                vm.availableTowns.unshift('All Towns');
            }, function (err) {
                console.error(err);
            });
        }
