...
var errors_array= [];
var fldNames = {'question':'Question', 'answer' : 'Answer' };
var fldType  = {'question':'textarea', 'answer' : 'textarea' };
...
$('.btn-edit').on('click', function (e) {
    ...
    $.ajax({
      url: dir_path+'site_users/modal_fetch_ajax',
      method:"POST",
      data: formData,
      contentType: false,
      cache: false,
...
function add_data_ajax(){
	   let user_id =  $('input[name=update_id]').val();
   	// let myFormData = $('#myForm').find(':input').serializeArray();
    ...
    $.ajax({
      url: dir_path +'site_users/modal_post_ajax', 
      method:"POST",
      data: formData,
      contentType: false,
      cache: false,
      ...
      success:function(data)
      {
        ...
        if( response['success'] == 1 ){
            ...
            /* Success */
            let target_url = 'create';
            let href= dir_path+"site_users/"+target_url+'/'+user_id+'/fun_facts';       
