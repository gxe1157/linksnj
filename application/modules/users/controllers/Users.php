...
class Users extends MX_Controller {
	...
	// public $mdl_name = 'Mdl_users';
	// public $main_controller = 'users';
	...
	public function signin(){
		...
		$data['message'] = message_box(
			(validation_errors() ? validation_errors() : ($this->ion_auth->errors()
								 ? $this->ion_auth->errors() : $this->session->flashdata('message'))),'danger');
	...
	function signup()
	{
		...
		// $this->form_validation->set_rules('username', $this->lang->line('create_user_username_label'), 'trim|required|min_length[4]|alpha_numeric|is_unique[users.username]');
		...
		if ($this->form_validation->run() == true)
		{
			...
			$group = array('4'); // Sets user to client.
		}
		...
		else
		{
			...
			//set the flash data error message if there is one
			$data['message'] = message_box((validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message'))),'danger');

			/* get  field information */
			$data = $this->_update_data($data);

			$data['view_module']="users";
		    $data['page_url'] = "signup";
		    $this->load->module('templates');
		    $this->templates->public_main($data);
	...
	public function favorites(){
    		...
    		$data['page_title'] = "My Saved Homes";
	...
	public function profile(){
	    ...
	    if( ($submit != "Submit") ) {
			...
			/* get  field information */
			$data['user'] = $this->_update_data($data, true);
	    }		
